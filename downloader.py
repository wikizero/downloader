...
class Downloader():
    ...
    def get_range(self, start=0):
        ...
        """
        lst = range(start, self.file_size, self.chunk_size)
    ...
    def download(self):
        ...
        # 初始化进度条
        pbar = tqdm(total=self.file_size,
                    initial=start,
                    unit='B',
                    unit_scale=True,
                    desc=self.filename,
        ...
        with ThreadPoolExecutor(max_workers=5) as pool:
            res = [pool.submit(self.download_by_piece, r) for r in self.get_range(start=start)]
